...
//       ) : (
//     <div className="border border-gray-200 rounded-xl shadow bg-white overflow-hidden">
//   <div className="overflow-y-auto max-h-98">
//     <table className="min-w-full text-sm">
//       <thead className="sticky top-0 bg-gradient-to-r from-blue-50 to-blue-100 text-gray-700 uppercase text-xs font-semibold z-10">
//         <tr>
...
//       </div>
//     </div>
<div className="p-4 sm:p-6 min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 py-20 md:py-0">
  {/* Header */}
  <div className="md:flex hidden flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
    <h2 className="text-xl sm:text-2xl font-bold text-gray-700 tracking-wide">
      👥 Search / Browse Users
    </h2>
    <Topbar />
  </div>
  ...
  {/* Search Bar */}
  <div className="flex sm:flex-row md:gap-3 gap-1 mb-6">
    <div className="relative md:flex-1">
      <Search className="absolute left-3 top-3 text-gray-400" size={18} />
      <input
        className="md:w-full border border-gray-300 rounded-xl pl-10 pr-4 py-2   text-sm shadow-sm focus:ring-1 focus:ring-blue-400 outline-none"
        value={q}
        onChange={(e) => setQ(e.target.value)}
        placeholder="Search by name, force no, etc..."
      />
    ...
    <div className="flex gap-2">
      <button
        onClick={() => load(q)}
        className="flex items-center justify-center gap-2 bg-blue-500 text-white px-4 sm:px-5 py-1  md:py-2 rounded-xl shadow hover:bg-blue-600 transition w-full sm:w-auto"
      >
        <Search size={16} /> Search
      </button>
      <button
        ...
        className="flex items-center justify-center gap-2 bg-gray-200 px-4 sm:px-5 py-1 md:py-2 rounded-xl shadow hover:bg-gray-300 transition w-full sm:w-auto"
      >
        <RotateCcw size={16} /> Reset
      </button>
    </div>
...
<div className="border border-gray-200 rounded-xl shadow bg-white">
  {/* Wrapper only for horizontal scroll */}
  ...
  {/* Action Buttons */}
  <div className="mt-4 md:grid grid-cols-1 flex sm:grid-cols-2 lg:grid-cols-3 gap-4">
    <button
      onClick={() => {
        if (selectedRow !== null) setOpenModal(true);
      }}
      className="flex items-center justify-center gap-2 bg-gray-300 py-2 px-4 rounded-xl shadow hover:bg-gray-200 cursor-pointer transition"
    >
      <Edit size={18} /> View / Edit
    </button>
    <ViewModel
    ...
    <button
      ...
      className="flex items-center justify-center gap-2 bg-gray-300 md:py-3 py-2 px-4  rounded-xl shadow hover:bg-gray-200 cursor-pointer transition"
    >
      <Calendar size={18} /> Leaves...
    </button>
    <LeaveModal
    ...
    <button
      onClick={() => document.getElementById("fileInput").click()}
      className="flex items-center justify-center gap-2 bg-green-500 text-white md:py-3 py-2 px-4  rounded-xl shadow hover:bg-green-600 cursor-pointer transition"
    >
      <Upload size={18} /> Import Users
    </button>
  </div>
