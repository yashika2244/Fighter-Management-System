...
import PageCard from "../../components/PageCard.jsx";
import { useNavigate } from "react-router-dom";
import AvailabilityOnDateModal from "./AvailabilityOnDate.jsx"; // modal import करो

export default function WriterHome() {
  const [q, setQ] = useState("");
  const [range, setRange] = useState("Last 30 days");
  const nav = useNavigate();
  const [openAvailability, setOpenAvailability] = useState(false); // modal state
    ...
    <div className="h-screen flex flex-col p-2">
      ...
      <div className="px-8">
        ...
        <div className="flex gap-6">
          <PageCard title="Feed New User" onClick={() => nav("/writer/new")} />
          <PageCard
            title="Look for any user"
            onClick={() => nav("/writer/search")}
          />
          {/* ab yahan page navigate na ho, popup open ho */}
          <PageCard
        ...
        <div className="mt-auto fixed bottom-6 left-72">
          <label className="mr-2 text-sm bg-gray-200 px-3 py-2 rounded-sm">
            Import Excel/CSV
          </label>
          <input type="file" onChange={() => nav("/writer/search?import=1")} />
        </div>
